...
class Task:
    ...
    def __init__(self, name=None, desc=None, source=[], target=[], rule=[], depend=[], resource={}, always=False, no_timestamp=False, ignore_same_task=False):
        ...
        if isinstance(rule, basestring):
            rule = rule.split('\n')
        if isinstance(depend, basestring):
            depend = depend.split()
        if name is None:
            name = '; '.join(rule)
        if desc is None:
...
class TaskGraph:
    ...
    def __make_prev_next_tasks(self):
        ...
        for task_id, task in enumerate(self.task_list):
            for source in task.source:
                self.prev_tasks[task_id].extend(prev_tasks.get(source, []))
            for target in task.target:
                self.next_tasks[task_id].extend(next_tasks.get(target, []))
...
class Scheduler:
    ...
    def run(self):
        ...
        worker_queue = range(self.num_jobs)
...
class Workflow:
    def set_options(self, num_jobs = None,
        ...
        if dependency_graph is not None:
            if not isinstance(dependency_graph, basestring): raise ValueError("dependency_graph must be string")
        ...
        if goal_targets is not None:
            if not (isinstance(goal_targets, list) and all([isinstance(x, basestring) for x in goal_targets])):
